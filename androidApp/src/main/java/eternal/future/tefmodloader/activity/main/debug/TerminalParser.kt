package eternal.future.tefmodloader.activity.main.debug
...
import eternal.future.tefmodloader.utils.SilkCasket
    ...
    return when (command) {
            ...
            "install" -> {
                ...
                return when (installType.lowercase()) {
                    "mod" -> {
                        try {
                            val targetDir = options["-target"] ?: File(AppConf.PATH_EFMOD, UUID.randomUUID().toString()).path
                    ...
                    "loader" -> {
                        try {
                            val targetDir = options["-target"] ?: File(AppConf.PATH_EFMOD_LOADER, UUID.randomUUID().toString()).path
